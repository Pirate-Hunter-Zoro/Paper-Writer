...
UNIT="${1:?usage: run.sh <author|builder>}"
...
# Prefer a pinned interpreter if one is configured; fall back to the system python3.
# Nothing outside the standard library is imported, so this is a convenience rather
# than a requirement.
if [ -n "${PAPER_PYTHON:-}" ] && [ -x "$PAPER_PYTHON" ]; then
    PYTHON="$PAPER_PYTHON"
else
    PYTHON="$(command -v python3)"
fi

# -m from the repo root, so `import paperwriter` resolves with no PYTHONPATH games.
exec "$PYTHON" -m "paperwriter.daemons.$UNIT"
