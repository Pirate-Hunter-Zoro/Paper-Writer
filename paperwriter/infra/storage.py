...
Every artifact the harness commits — section prose, an outline, the frozen evidence,
the ledger, the assembled manuscript, the delivered copy — is written into a hidden
staging path and then atomically renamed into its final location, so no downstream
watcher (a sync client included) ever observes a half-written file.
...
def sha256_file(path):
    """Content hash — makes delivery content-addressed and therefore a
