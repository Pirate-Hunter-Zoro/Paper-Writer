"""Reading a folder that iCloud owns.

The drop folder lives inside iCloud Drive so a phone can control the fleet, which
means the local filesystem is no longer the authority on what is there. Two failure
modes matter, and both are silent:

**Eviction.** iCloud reclaims space by replacing a file's contents with a placeholder.
`foo.md` disappears from the directory and `.foo.md.icloud` takes its place, so a
plain `*.md` glob simply stops seeing a job that is definitely still there. The fix
is to notice placeholders and ask for them back (`brctl download`), then wait — the
file reappears under its real name once materialised.

**Partial arrival.** A file being written by an editor, or landing from the phone, can
be observed mid-write. Admitting a truncated prompt is worse than not seeing it: the
research stage would freeze canon against half a brief and the job would look
successful. So a candidate must be non-empty and unchanged for `INBOX_SETTLE_SEC`
before anyone is allowed to read it.

Neither guard needs iCloud specifically — an editor writing in place on a local disk
hits the second one too — but a synced folder is where they stop being theoretical.
"""
...
# trying afterwards.
#
# Earned on 2026-08-04, and the worst failure this system has produced. Enumerating an
# iCloud directory from a launchd agent is denied by macOS privacy protection — but
...
SCAN_TIMEOUT_SEC = int(os.environ.get("FANFIC_SCAN_TIMEOUT_SEC", "15"))
SCAN_BACKOFF_SEC = int(os.environ.get("FANFIC_SCAN_BACKOFF_SEC", "300"))
...
def _with_deadline(work, timeout):
    ...
    worker = threading.Thread(target=run, daemon=True, name="icloud-io")
    worker.start()
    worker.join(timeout)
