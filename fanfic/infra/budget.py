"""The spend ceiling, and the usage meter.

The engine checks remaining API budget before starting a unit of work; exhausted,
it idles rather than failing, so a runaway can never silently drain the enterprise
spend. The ceiling is a JSON file under the user config dir (the Media-Syncer
...
def record_usage(usd, label=""):
    ...
    **This is a usage meter, not a bill.** The `claude` CLI reports `total_cost_usd` in
    ...
    must never be the thing that fails a chapter."""
    try:
        amount = float(usd)
...
def record_image(series_id, label=""):
    """Count one generated image against this series' picture budget.

    Images are the only line item in this fleet that moves real money — prose and
    judgment run on a seat — so this is the one meter that is genuinely a bill, and
    it is the one the operator's "about $5 a novel" constraint is actually about.
    Every generated picture is counted, including one regenerated after a vision
    critique rejected it: the vendor charges for the rejected render too, and a budget
    that only counts the keepers is a budget that quietly overruns by the reject rate.

    Returns the new count for this series. Swallows errors like the other meters —
    accounting must never be the thing that fails a book."""
    path = paths.image_spend_log()
    try:
        path.parent.mkdir(parents=True, exist_ok=True)
        with path.open("a", encoding="utf-8") as handle:
            handle.write(json.dumps(
                {"series_id": series_id, "images": 1, "label": label}) + "\n")
    except OSError:
        return None
    return images_generated(series_id)


def images_generated(series_id):
    """How many images this series has generated so far."""
    path = paths.image_spend_log()
    total = 0
    try:
        for line in path.read_text(encoding="utf-8").splitlines():
            ...
            if record.get("series_id") == series_id:
                total += int(record.get("images") or 0)
    except OSError:
        return 0
    return total


def image_budget_remaining(series_id):
    """How many more renders this series may spend. Infinity when uncapped.

    A ceiling rather than an estimate. It used to be denominated in dollars, because
    pictures were the one thing this fleet genuinely paid for; they are drawn through
    a signed-in browser now and cost nothing but time, so the ceiling counts renders.

    What it protects against is unchanged: a slot that keeps failing burns renders,
    and a book has hundreds of slots. Images are never skipped, so what a spent
    ceiling costs is TIME — the book holds in ILLUSTRATING with its slots queued, and
    raising the ceiling resumes it unattended."""
    cap = config.IMAGE_RENDER_BUDGET
    if not cap or cap <= 0:
        return float("inf")
    return max(0, int(cap) - images_generated(series_id))
