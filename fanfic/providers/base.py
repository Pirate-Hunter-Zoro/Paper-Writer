...
The fleet reaches two services — Claude for text, a browser session for pictures —
and this module holds the declarations they share. No I/O, so it can be read on its
own.
...
class Capability:
    ...
    def __init__(self, writes_own_file=False, supports_tools=False,
                 supports_web=False, supports_references=False):
        ...
        # Can it reach the live web? Research is impossible without this.
        self.supports_web = supports_web
        # Images only: can it condition on reference images (locked character sheets)?
        self.supports_references = supports_references


def is_transient(detail):
    """Whether a failure message looks like a retryable blip.

    Deliberately provider-agnostic: a dropped CLI stream and a browser that lost its
    page are the same class of problem, and the signature list is a denylist of
    network reality that both should share."""
    low = (detail or "").lower()
...
def file_contract(out_path, artifact, shape=None, oneshot=False):
    ...
    previous draft, greps the bible, and then lays a chapter down in eight appends pays
    for that whole accumulating transcript eight times over: a chapter draft measured at
