...
def _consecutive(record, now=None):
    ...
    every later write, so without this a book that stalled four times during research
    and then wrote thirty clean chapters would wait an hour before retrying its first
...
def _resumable_status(record):
    ...
    sent a book that stalled while outlining back to DRAFTING, where it would find no
    chapters to draft and walk out the other side of the book machine with an empty
    novel to bind."""
    status = record.get("status")
